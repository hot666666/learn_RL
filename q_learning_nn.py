...
class QLearningAgent:
    ...
    def update(self, state, action, reward, next_state, done):
        ...
        if done:  # 목표 상태에 도달(목표 상태에서의 Q 함수는 항상 0)
            ...
            next_q.unchain()  # next_q를 역전파 대상에서 제외(단지 레이블로 가정하는 T-target를 얻기위해 순전파 수행)

        target = reward + self.gamma * next_q
        
        # 현재 상태에서의 Q 함수 값(q)과 T(target)와 오차를 줄이는 방향으로 가중치 업데이트
        qs = self.qnet(state)
        q = qs[:, action]
        loss = F.mean_squared_error(target, q)

        # 역전파 → 매개변수 갱신
        self.qnet.cleargrads() # 이전 기울기 초기화
        loss.backward()  # 기울기 갱신
