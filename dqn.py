...
class DQNAgent:  
    # q_learning_nn.py에서 "경험 재생"과 "타깃 신경망 동기화" 기능 추가
    def __init__(self):
        self.gamma = 0.98
        self.lr = 0.0005
        self.epsilon = 0.1
    ...
    def get_action(self, state):
        if np.random.rand() < self.epsilon:
            ...
            state = state[np.newaxis, :]  # 모델에 입력할 수 있도록 차원 추가
            qs = self.qnet(state)
...
agent.epsilon = 0  # 학습이 끝난 에이전트에 탐욕 행동을 선택하도록 하여 플레이(무작위로 행동할 확률 ε을 0으로 설정)
state = env.reset()[0]
done = False
total_reward = 0
